...
static NSString *MacSchemeGraphicsBootstrapSource(void) {
    return @"(begin "
           ...
           "    (when (> h 512) "
           "      (assertion-violation 'sprite-from-rows! \"sprite height must be 512 rows or less\" rows)) "
           "    (when (> w 512) "
           "      (assertion-violation 'sprite-from-rows! \"sprite width must be 512 columns or less\" rows)) "
           "    (for-each (lambda (row) "
